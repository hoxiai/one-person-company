...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "space-y-6" }, _attrs))}><div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3"><div><div class="flex items-center gap-2"><span class="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 text-sm">`);
      _push(ssrRenderComponent(_component_UIcon, {
        name: "ph:wrench-bold",
        class: "w-4 h-4"
      }, null, _parent));
      _push(`</span><h2 class="text-lg md:text-xl font-bold tracking-tight text-slate-900 dark:text-white"> \u5DE5\u5177\u7BB1 </h2></div><p class="mt-1 text-xs text-slate-500 dark:text-slate-400"> \u5F00\u53D1\u3001\u90E8\u7F72\u3001\u6536\u6B3E\u548C AI \u5DE5\u5177\u3002 </p></div>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<span${_scopeId}>\u5168\u90E8 ${ssrInterpolate(unref(tools).length)} \u4E2A</span>`);
            ...
            return [
              createVNode("span", null, "\u5168\u90E8 " + toDisplayString(unref(tools).length) + " \u4E2A", 1),
      ...
      ssrRenderList(featuredTools.value, (tool) => {
        ...
        _push(`</span><div><h3 class="text-sm font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">${ssrInterpolate(tool.name)}</h3><span class="text-[10px] text-slate-400 font-mono dark:text-slate-500">${ssrInterpolate(tool.categoryLabel)}</span></div></div>`);
        if (tool.badge) {
