...
import { f as formatScore, a as formatPrice, b as formatContext, c as formatTps } from './useHoxiModels-CB4dunlj.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><div><div class="flex items-center justify-between gap-2 mb-3"><div class="flex items-center gap-2"><span class="${ssrRenderClass([__props.rank === 1 ? "bg-amber-100 text-amber-800 border border-amber-300" : __props.rank === 2 ? "bg-slate-100 text-slate-700 border border-slate-300" : __props.rank === 3 ? "bg-amber-50 text-amber-900 border border-amber-200" : "bg-slate-50 dark:bg-slate-800 text-slate-400 border border-slate-100 dark:border-slate-700", "w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center font-mono tabular-nums"])}">`);
      if (__props.rank === 1) {
      ...
      if (__props.model.isFree) {
        _push(`<span class="px-2 py-0.5 rounded-md text-[10px] bg-emerald-50 text-emerald-700 font-medium"> \u514D\u8D39 </span>`);
      } else if (__props.model.reasoning) {
        _push(`<span class="px-2 py-0.5 rounded-md text-[10px] bg-blue-50 text-blue-700 font-medium"> \u6DF1\u5EA6\u601D\u8003 </span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><div class="mt-4 grid grid-cols-2 gap-2 text-xs bg-slate-50 dark:bg-slate-800/40 rounded-xl p-3 border border-slate-100 dark:border-slate-800"><div><span class="text-[10px] text-slate-400 block">\u7EFC\u5408\u6218\u529B</span><span class="font-mono font-bold text-slate-900 dark:text-white text-base tabular-nums">${ssrInterpolate(unref(formatScore)(__props.model.scores.overall))}</span></div><div><span class="text-[10px] text-slate-400 block">\u6DF7\u5408\u5355\u4EF7</span><div class="flex items-baseline gap-0.5"><span class="${ssrRenderClass([__props.model.blendedPrice <= 2 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-900 dark:text-white", "font-mono font-bold text-base tabular-nums"])}"> \xA5${ssrInterpolate(unref(formatPrice)(__props.model.blendedPrice))}</span><span class="text-[10px] text-slate-400">/M</span></div></div></div><div class="mt-3 space-y-1.5 text-xs text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-2.5"><div class="flex items-center justify-between"><span>\u8F93\u5165 / \u8F93\u51FA\uFF1A</span><span class="font-mono tabular-nums text-slate-700 dark:text-slate-300">${ssrInterpolate(currencySymbol(__props.model.price.currency))}${ssrInterpolate(unref(formatPrice)(__props.model.price.input))} / ${ssrInterpolate(currencySymbol(__props.model.price.currency))}${ssrInterpolate(unref(formatPrice)(__props.model.price.output))}</span></div><div class="flex items-center justify-between"><span>\u4E0A\u4E0B\u6587\u7A97\u53E3\uFF1A</span><span class="font-mono tabular-nums text-slate-700 dark:text-slate-300">${ssrInterpolate(unref(formatContext)(__props.model.contextWindow))}</span></div><div class="flex items-center justify-between"><span>\u8F93\u51FA\u541E\u5410\u901F\u5EA6\uFF1A</span><span class="font-mono tabular-nums text-slate-700 dark:text-slate-300">${ssrInterpolate(unref(formatTps)((_b = __props.model.perf) == null ? void 0 : _b.tps))} TPS </span></div></div></div><div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end">`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: unref(localePath)(`/models/${__props.model.slug}`),
