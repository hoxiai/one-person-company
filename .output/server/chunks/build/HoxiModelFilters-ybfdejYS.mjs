import { e as useI18n, b as _sfc_main$E } from './server.mjs';
import __nuxt_component_3$1 from './HoxiVendorDot-BQ5SFhuO.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_HoxiVendorDot = __nuxt_component_3$1;
      ...
      if (__props.searchQuery) {
        _push(`<button type="button" class="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-0.5">`);
        _push(ssrRenderComponent(_component_UIcon, {
          name: "ph:x-bold",
          class: "w-3.5 h-3.5"
        }, null, _parent));
      ...
      if (hasActiveFilter.value) {
        _push(`<button type="button" class="text-xs text-slate-400 underline decoration-slate-200 dark:decoration-slate-700 underline-offset-4 transition-colors hover:text-blue-500">${ssrInterpolate(_ctx.$t("hoxi.models.filters.reset"))}</button>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div>`);
...
const __nuxt_component_3 = Object.assign(_sfc_main, { __name: "HoxiModelFilters" });
