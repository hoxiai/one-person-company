...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const podiumModels = computed(() => {
      ...
      const overallModel = list.find((m) => m.slug === "claude-3-7-sonnet") || [...list].sort((a, b) => (b.scores.overall || 0) - (a.scores.overall || 0))[0];
      const valueModel = list.find((m) => m.slug === "deepseek-v3") || [...list].sort((a, b) => (b.valueIndex || 0) - (a.valueIndex || 0))[0];
      const speedModel = list.find((m) => m.slug === "gemini-2-5-flash") || list.find((m) => m.slug === "glm-4-flash") || [...list].sort((a, b) => {
        var _a, _b;
      ...
      if (overallModel) {
        result.push({
          ...
          verdict: "\u590D\u6742\u4EE3\u7801\u91CD\u6784\u4E0E\u6DF1\u5EA6\u903B\u8F91\u9996\u9009\uFF0C\u5B9E\u6D4B\u51C6\u786E\u7387\u4E0E\u63A8\u7406\u4E0A\u9650\u9065\u9065\u9886\u5148\u3002",
          tip: "\u4E3B\u529B\u7F16\u7A0B\u9996\u9009"
        });
      }
      if (valueModel) {
        result.push({
          ...
          verdict: "\u540C\u7B49\u65D7\u8230\u80FD\u529B\u4E0B\u4EF7\u683C\u4F4E\u81F3 \xA51.5/M\uFF0C\u65E5\u5E38\u5F00\u53D1\u4E0E\u9AD8\u5E76\u53D1\u8DD1\u6279\u7684\u964D\u672C\u6740\u624B\u950F\u3002",
          tip: "\u5E73\u66FF\u964D\u8D39 85%"
        });
      }
      if (speedModel) {
        result.push({
          ...
          verdict: "\u8D85\u4F4E\u9996\u5B57\u5EF6\u8FDF\u4E0E\u767E\u6BEB\u79D2\u54CD\u5E94\uFF0C\u767E\u4E07\u957F\u4E0A\u4E0B\u6587\u4E0E\u6D77\u91CF\u68C0\u7D22\u7684\u8F7B\u91CF\u5229\u5668\u3002",
          tip: "\u9AD8\u541E\u5410\u957F\u6587\u672C"
        });
      }
      return result;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (podiumModels.value.length) {
        ...
        ssrRenderList(podiumModels.value, (item) => {
          ...
          _push(`<p class="mt-2.5 text-xs text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-2">${ssrInterpolate(item.verdict)}</p><div class="mt-4 grid grid-cols-2 gap-2 text-xs bg-slate-50 dark:bg-slate-800/50 rounded-xl p-2.5 border border-slate-100 dark:border-slate-800"><div><span class="text-[10px] text-slate-400 block">\u7EFC\u5408\u6218\u529B</span><span class="font-mono font-bold text-slate-900 dark:text-white text-sm tabular-nums">${ssrInterpolate(unref(formatScore)(item.model.scores.overall))}</span></div><div><span class="text-[10px] text-slate-400 block">\u6DF7\u5408\u5355\u4EF7</span><span class="${ssrRenderClass([item.model.blendedPrice <= 2 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-900 dark:text-white", "font-mono font-bold text-sm tabular-nums"])}"> \xA5${ssrInterpolate(unref(formatPrice)(item.model.blendedPrice))}</span><span class="text-[10px] text-slate-400">/M</span></div><div><span class="text-[10px] text-slate-400 block">\u4E0A\u4E0B\u6587</span><span class="font-mono text-slate-700 dark:text-slate-300 tabular-nums">${ssrInterpolate(unref(formatContext)(item.model.contextWindow))}</span></div><div><span class="text-[10px] text-slate-400 block">\u8F93\u51FA\u901F\u5EA6</span><span class="font-mono text-slate-700 dark:text-slate-300 tabular-nums">${ssrInterpolate(unref(formatTps)((_a = item.model.perf) == null ? void 0 : _a.tps))} TPS </span></div></div></div><div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs"><span class="text-[11px] text-slate-400">${ssrInterpolate(item.tip)}</span>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: unref(localePath)(`/models/${item.model.slug}`),
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<span${_scopeId}>\u5B9E\u6D4B\u8BE6\u60C5</span>`);
