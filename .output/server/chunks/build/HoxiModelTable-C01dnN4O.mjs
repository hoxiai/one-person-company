import { aL as useLocaleRouter, e as useI18n, a as __nuxt_component_3$1 } from './server.mjs';
import __nuxt_component_4$1 from './HoxiVendorDot-F2b2YLsh.mjs';
import __nuxt_component_2 from './HoxiValueBar-w_ixP6gD.mjs';
...
import { f as formatScore, a as formatPrice, c as formatTps, d as formatLatency, b as formatContext } from './useHoxiModels-D3S6mr-U.mjs';
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.models, (model, index) => {
        ...
        if (__props.selectable) {
          _push(`<input type="checkbox" class="rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 cursor-pointer w-3.5 h-3.5"${ssrIncludeBooleanAttr((_c = __props.selectedSlugs) == null ? void 0 : _c.includes(model.slug)) ? " checked" : ""}>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<span class="${ssrRenderClass([index === 0 ? "text-amber-500 font-extrabold" : index === 1 ? "text-slate-400 font-bold" : index === 2 ? "text-amber-700 font-bold" : "text-slate-300 dark:text-slate-600 font-normal", "w-5 shrink-0 tabular-nums text-xs flex items-center justify-center font-mono"])}">`);
        if (index === 0) {
        ...
        _push(`<!--]-->`);
        if (model.isFree) {
          _push(`<span class="px-1.5 py-0.2 rounded text-[10px] bg-emerald-50 text-emerald-700 font-medium shrink-0">\u514D\u8D39</span>`);
        } else if (model.reasoning) {
          _push(`<span class="px-1.5 py-0.2 rounded text-[10px] bg-blue-50 text-blue-700 font-medium shrink-0">\u601D\u8003</span>`);
        } else {
          _push(`<!---->`);
        }
...
const __nuxt_component_4 = Object.assign(_sfc_main, { __name: "HoxiModelTable" });
