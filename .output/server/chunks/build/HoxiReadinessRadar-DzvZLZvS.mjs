  ...
  setup(__props) {
    ...
    const userVertexPoints = computed(() => {
      return props.dimensions.map((dim, i) => {
        var _a;
        const angle = (_a = angles[i]) != null ? _a : 0;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList([0.2, 0.4, 0.6, 0.8, 1], (step) => {
        _push(`<polygon${ssrRenderAttr("points", getPolygonPoints(step))} class="fill-transparent stroke-slate-200 dark:stroke-slate-700" stroke-width="1" stroke-dasharray="2,2"></polygon>`);
      });
      _push(`<!--]--><!--[-->`);
      ssrRenderList(axisPoints.value, (axis, i) => {
        _push(`<line${ssrRenderAttr("x1", center.x)}${ssrRenderAttr("y1", center.y)}${ssrRenderAttr("x2", axis.x)}${ssrRenderAttr("y2", axis.y)} class="stroke-slate-200 dark:stroke-slate-700" stroke-width="1"></line>`);
      });
      ...
      ssrRenderList(labelPoints.value, (label, i) => {
        _push(`<g${ssrRenderAttr("transform", `translate(${label.x}, ${label.y})`)}${ssrRenderAttr("text-anchor", label.anchor)} class="select-none"><text y="-4" class="text-[12px] font-medium fill-slate-700 font-sans dark:fill-slate-300">${ssrInterpolate(label.name)}</text><text y="12" class="text-[11px] font-mono tabular-nums font-semibold fill-blue-600 dark:fill-blue-400">${ssrInterpolate(label.score)}\u5206 </text></g>`);
      });
      _push(`<!--]--></svg></div></div>`);
    };
  }
