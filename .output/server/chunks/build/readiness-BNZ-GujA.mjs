...
import __nuxt_component_2 from './HoxiReadinessProgress-D7zePPDj.mjs';
import { D as DIMENSIONS, _ as __nuxt_component_3, R as ROLE_QUESTIONS_MAP, a as READINESS_TIERS, A as ARCHETYPES, B as BLINDSPOT_PRESCRIPTIONS, b as ROLES, c as ROLE_ADVICES } from './HoxiReadinessQuestion-CwH_pQJv.mjs';
import __nuxt_component_4 from './HoxiReadinessReport-DBWVzdPW.mjs';
...
import 'unhead/utils';
import './HoxiReadinessRadar-DzvZLZvS.mjs';
import './useHoxiCommunity-BDhMDRk9.mjs';
...
function useReadinessQuiz() {
  ...
  const results = computed(() => {
    ...
    const tier = (_b = READINESS_TIERS.find((t) => overallScore >= t.minScore && overallScore <= t.maxScore)) != null ? _b : READINESS_TIERS[3];
    ...
    const strongestDimension = (_c = sortedByScore[0]) != null ? _c : dimensions[0];
    const weakestDimension = (_d = sortedByScore[sortedByScore.length - 1]) != null ? _d : dimensions[0];
    ...
    const blindspots = targetBlindspotKeys.map((k) => BLINDSPOT_PRESCRIPTIONS[k]).filter(Boolean);
    const role = selectedRole.value;
    const roleMeta = (_e = ROLES[role]) != null ? _e : ROLES["developer"];
    const roleAdvice = (_f = ROLE_ADVICES[role]) != null ? _f : ROLE_ADVICES["developer"];
    return {
      role,
      roleMeta,
      roleAdvice,
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_HoxiReadinessProgress = __nuxt_component_2;
      const _component_HoxiReadinessQuestion = __nuxt_component_3;
      const _component_HoxiReadinessReport = __nuxt_component_4;
            ...
            return [
              createVNode("div", { class: "py-8 md:py-16" }, [
                !isStarted.value && !unref(isCompleted) ? (openBlock(), createBlock("div", {
                  ...
                  unref(currentQuestion) ? (openBlock(), createBlock(_component_HoxiReadinessQuestion, {
                    key: 0,
                    question: unref(currentQuestion),
                    "selected-option-index": unref(answers)[unref(currentQuestion).id],
                    "current-index": unref(currentIndex),
                    ...
                    onNext: handleNext,
                    onPrev: unref(prevQuestion)
                  }, null, 8, ["question", "selected-option-index", "current-index", "total-questions", "onSelect", "onPrev"])) : createCommentVNode("", true)
