import { f as useI18n, g as useFormatTime, s as useRouter, z as useFetch, y as usePagination, t as useConfirm, i as useAdminPermissions, l as _sfc_main$B, b as _sfc_main$G, o as _sfc_main$x, d as _sfc_main$k, B as _sfc_main$n, h as useToast, q as _sfc_main$s } from './server.mjs';
...
const buildStatsOverviewCards = (overview, translate, formatNumber, formatPercent) => {
  ...
  return [
    ...
    },
    {
      ...
      value: formatNumber((_a = overview.uniqueIps) != null ? _a : overview.todayVisitors),
      icon: "ph:globe",
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const isTrendExpanded = ref(false);
    const displayedTrend = computed(() => {
      const list = trend.value;
      if (isTrendExpanded.value || list.length <= defaultTrendLimit) {
        return list;
      }
      return list.slice(-defaultTrendLimit);
    });
    watch(preset, () => {
      isTrendExpanded.value = false;
    });
    ...
    const visitorsUrl = computed(
      () => {
        ...
        if (modalSourceType.value) {
          url += `&sourceType=${encodeURIComponent(modalSourceType.value)}`;
        }
        if (modalSourceName.value) {
          url += `&source=${encodeURIComponent(modalSourceName.value)}`;
        }
        return url;
      }
    );
    ...
    function formatSourceLabel(value) {
      ...
      if (normalized === "ai") return t("admin.stats.ai") || "AI";
      if (normalized === "campaign") return t("admin.stats.campaign");
      if (normalized === "affiliate") return t("admin.stats.affiliate") || "Affiliate";
      if (normalized === "email") return t("admin.stats.email") || "Email";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div><div class="grid grid-cols-1 xl:grid-cols-[1.5fr_1fr] gap-6 mb-8"><div class="bg-white dark:bg-[#121214] rounded-2xl border border-gray-200/60 dark:border-gray-800/50 shadow-sm p-6 flex flex-col justify-between"><div><div class="flex items-center justify-between mb-6"><div><div class="flex items-center gap-2"><h2 class="text-lg font-bold text-gray-900 dark:text-white">${ssrInterpolate(unref(t)("admin.stats.trafficTrend"))}</h2>`);
      if (unref(trend).length > defaultTrendLimit && !unref(isTrendExpanded)) {
        _push(ssrRenderComponent(_component_UBadge, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(unref(t)("admin.stats.recentDays", { count: defaultTrendLimit }))}`);
            } else {
              return [
                createTextVNode(toDisplayString(unref(t)("admin.stats.recentDays", { count: defaultTrendLimit })), 1)
              ];
            }
          }),
          _: 1
      ...
      _push(`</div><p class="text-sm text-gray-500 mt-1">${ssrInterpolate(unref(t)("admin.stats.trafficTrendSubtitle"))}</p></div>`);
      if (unref(trend).length > defaultTrendLimit) {
        _push(ssrRenderComponent(_component_UButton, {
          color: "neutral",
          variant: "ghost",
          size: "xs",
          icon: unref(isTrendExpanded) ? "ph:caret-up" : "ph:caret-down",
          class: "shrink-0",
          onClick: ($event) => isTrendExpanded.value = !unref(isTrendExpanded)
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(unref(isTrendExpanded) ? unref(t)("admin.stats.collapseTrend") : unref(t)("admin.stats.viewAllTrend", { count: unref(trend).length }))}`);
            } else {
              return [
                createTextVNode(toDisplayString(unref(isTrendExpanded) ? unref(t)("admin.stats.collapseTrend") : unref(t)("admin.stats.viewAllTrend", { count: unref(trend).length })), 1)
              ];
            }
          }),
          _: 1
      ...
      _push(`</div><div class="${ssrRenderClass(unref(isTrendExpanded) && unref(trend).length > 10 ? "max-h-[460px] overflow-y-auto pr-1 space-y-3" : "space-y-3")}"><!--[-->`);
      ssrRenderList(unref(displayedTrend), (item) => {
      ...
      _push(`<!--]--></div></div><div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-800/60 flex flex-wrap items-center justify-between gap-3 text-xs text-gray-500"><div class="flex items-center gap-6"><div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-cyan-500"></div> ${ssrInterpolate(unref(t)("admin.stats.pageViews"))}</div><div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-purple-500"></div> ${ssrInterpolate(unref(t)("admin.stats.uniqueVisitors"))}</div><div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-emerald-500"></div> ${ssrInterpolate(unref(t)("admin.stats.paidVisitors"))}</div></div>`);
      if (unref(trend).length > defaultTrendLimit) {
        _push(ssrRenderComponent(_component_UButton, {
          color: "neutral",
          variant: "link",
          size: "xs",
          icon: unref(isTrendExpanded) ? "ph:caret-up" : "ph:caret-down",
          class: "p-0 text-xs text-gray-500 hover:text-gray-900 dark:hover:text-white",
          onClick: ($event) => isTrendExpanded.value = !unref(isTrendExpanded)
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
      ...
      _push(`<!--]--></div></div></div><div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8"><div class="bg-white dark:bg-[#121214] rounded-2xl border border-gray-200/60 dark:border-gray-800/50 shadow-sm p-6"><div class="flex items-center justify-between mb-5"><div><h2 class="text-lg font-bold text-gray-900 dark:text-white">${ssrInterpolate(unref(t)("admin.stats.firstTouch"))}</h2><p class="text-sm text-gray-500 mt-1">${ssrInterpolate(unref(t)("admin.stats.firstTouchSubtitle"))}</p></div></div><div class="space-y-4"><!--[-->`);
      ssrRenderList(unref(firstTouchSources), (item) => {
        _push(`<div class="${ssrRenderClass([item.count > 0 ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/40 group" : "", "space-y-1.5 p-2 -mx-2 rounded-xl transition-colors"])}"><div class="flex items-center justify-between text-sm"><div class="flex items-center gap-2 max-w-[70%] truncate"><span class="text-gray-700 dark:text-gray-200 font-medium truncate group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors"${ssrRenderAttr("title", item.label)}>${ssrInterpolate(formatSourceLabel(item.label))}</span>`);
        if (item.paid > 0) {
          _push(ssrRenderComponent(_component_UBadge, {
            color: "success",
            variant: "subtle",
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(item.paid)} ${ssrInterpolate(unref(t)("admin.stats.paidOrders") || "paid")} \xB7 $${ssrInterpolate(item.revenue)} (${ssrInterpolate(item.conversionRate)}%) `);
              } else {
                return [
                  createTextVNode(toDisplayString(item.paid) + " " + toDisplayString(unref(t)("admin.stats.paidOrders") || "paid") + " \xB7 $" + toDisplayString(item.revenue) + " (" + toDisplayString(item.conversionRate) + "%) ", 1)
                ];
              }
            }),
            _: 2
        ...
        _push(`</div><div class="flex items-center gap-1.5 text-right text-gray-900 dark:text-white font-medium whitespace-nowrap text-xs"><span>${ssrInterpolate(formatNumber(item.count))} <span class="text-gray-400 font-normal">(${ssrInterpolate(formatPercent(item.percentage))})</span></span>`);
        if (item.count > 0) {
          _push(ssrRenderComponent(_component_UIcon, {
            name: "ph:caret-right",
        ...
        _push(`</div></div><div class="h-1.5 rounded-full bg-gray-100 dark:bg-white/5 overflow-hidden"><div class="h-full rounded-full bg-purple-500" style="${ssrRenderStyle({ width: `${item.percentage}%` })}"></div></div></div>`);
      });
      _push(`<!--]--></div></div><div class="bg-white dark:bg-[#121214] rounded-2xl border border-gray-200/60 dark:border-gray-800/50 shadow-sm p-6"><div class="flex items-center justify-between mb-5"><div><h2 class="text-lg font-bold text-gray-900 dark:text-white">${ssrInterpolate(unref(t)("admin.stats.lastTouch"))}</h2><p class="text-sm text-gray-500 mt-1">${ssrInterpolate(unref(t)("admin.stats.lastTouchSubtitle"))}</p></div></div><div class="space-y-4"><!--[-->`);
      ssrRenderList(unref(lastTouchSources), (item) => {
        ...
        _push(`</div></div><div class="h-1.5 rounded-full bg-gray-100 dark:bg-white/5 overflow-hidden"><div class="h-full rounded-full bg-cyan-500" style="${ssrRenderStyle({ width: `${item.percentage}%` })}"></div></div></div>`);
      });
      ...
      ssrRenderList(unref(sourceCategories), (item) => {
        _push(`<div class="${ssrRenderClass([item.count > 0 ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/40 group" : "opacity-60", "flex items-center justify-between text-sm py-2 px-2.5 -mx-2.5 rounded-xl border-b border-gray-50 dark:border-gray-900/50 last:border-none transition-colors"])}"><div class="flex items-center gap-2"><span class="text-gray-700 dark:text-gray-200 font-medium group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">${ssrInterpolate(formatSourceLabel(item.label))}</span>`);
        if (item.paid > 0) {
          _push(ssrRenderComponent(_component_UBadge, {
            color: "success",
            variant: "subtle",
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(item.paid)} \u5355 \xB7 $${ssrInterpolate(item.revenue)}`);
              } else {
                return [
                  createTextVNode(toDisplayString(item.paid) + " \u5355 \xB7 $" + toDisplayString(item.revenue), 1)
                ];
              }
            }),
            _: 2
        ...
        _push(`</div><div class="flex items-center gap-2"><span class="text-gray-900 dark:text-white tabular-nums">${ssrInterpolate(formatNumber(item.count))} \xB7 ${ssrInterpolate(formatPercent(item.percentage))}</span>`);
        if (item.count > 0) {
          _push(ssrRenderComponent(_component_UIcon, {
            name: "ph:caret-right",
      ...
      ssrRenderList(unref(externalSources), (item) => {
        ...
        if (item.paid > 0) {
          _push(ssrRenderComponent(_component_UBadge, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(item.paid)} ${ssrInterpolate(unref(t)("admin.stats.paidOrders") || "paid")} \xB7 $${ssrInterpolate(item.revenue)}`);
              } else {
                return [
                  createTextVNode(toDisplayString(item.paid) + " " + toDisplayString(unref(t)("admin.stats.paidOrders") || "paid") + " \xB7 $" + toDisplayString(item.revenue), 1)
                ];
              }
            }),
            _: 2
        ...
        _push(`</div></div><div class="h-1.5 rounded-full bg-gray-100 dark:bg-white/5 overflow-hidden"><div class="h-full rounded-full bg-sky-500" style="${ssrRenderStyle({ width: `${item.percentage}%` })}"></div></div></div>`);
      });
          ...
          if (_push2) {
            ...
            if (unref(modalSource) === "visitors") {
              ...
              ssrRenderList(unref(modalRows), (item) => {
                ...
                _push2(`</td><td class="py-3 pr-4 text-gray-500 dark:text-gray-400 font-mono text-xs"${_scopeId}>${ssrInterpolate(item.ip || "Local")}</td><td class="py-3 pr-4 whitespace-nowrap"${_scopeId}><div class="flex items-center gap-1.5"${_scopeId}>`);
                _push2(ssrRenderComponent(_component_UBadge, {
                  color: unref(getSourceTypeColor)(item.firstSourceType),
                  variant: "subtle",
                  size: "xs"
                }, {
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    if (_push3) {
                      _push3(`${ssrInterpolate(formatSourceLabel(item.firstSourceType || "direct"))}`);
                    } else {
                      return [
                        createTextVNode(toDisplayString(formatSourceLabel(item.firstSourceType || "direct")), 1)
                      ];
                    }
                  }),
                  _: 2
                }, _parent2, _scopeId));
                if (item.firstSource && item.firstSource !== "direct" && item.firstSource !== item.firstSourceType) {
                  _push2(`<span class="text-gray-700 dark:text-gray-200 text-xs font-medium max-w-[120px] truncate"${ssrRenderAttr("title", item.firstSource)}${_scopeId}>${ssrInterpolate(item.firstSource)}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                if (item.firstCampaign) {
                  _push2(`<span class="text-gray-400 dark:text-gray-500 text-[11px] font-mono max-w-[80px] truncate"${ssrRenderAttr("title", item.firstCampaign)}${_scopeId}> (${ssrInterpolate(item.firstCampaign)}) </span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></td><td class="py-3 pr-4 text-gray-600 dark:text-gray-300 whitespace-nowrap"${_scopeId}>${ssrInterpolate(item.country)}</td><td class="py-3 pr-4 text-gray-600 dark:text-gray-300 whitespace-nowrap"${_scopeId}>${ssrInterpolate(item.deviceType)}</td><td class="py-3 pr-4 text-gray-900 dark:text-white text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.pageViews))}</td><td class="py-3 pr-4 text-gray-600 dark:text-gray-300 text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.productViews))}</td><td class="py-3 pr-4 text-amber-600 dark:text-amber-300 text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.checkouts))}</td><td class="py-3 pr-4 text-emerald-600 dark:text-emerald-300 text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.paid))}</td><td class="py-3 pr-4 text-pink-600 dark:text-pink-300 text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.auth))}</td><td class="py-3 pr-4 text-gray-500 dark:text-gray-400 whitespace-nowrap text-xs"${_scopeId}>${ssrInterpolate(unref(formatDateTime)(item.lastSeenAt))}</td><td class="py-3 pr-4 text-right"${_scopeId}>`);
                _push2(ssrRenderComponent(_component_UButton, {
                  color: "neutral",
                  variant: "ghost",
                  icon: "ph:eye",
              ...
              _push2(`<table class="min-w-full text-sm"${_scopeId}><thead class="sticky top-0 bg-white dark:bg-[#121214]"${_scopeId}><tr class="text-left text-gray-500 border-b border-gray-200 dark:border-gray-800"${_scopeId}><th class="py-3 px-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.ip"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.source"))}</th><th class="py-3 pr-4 whitespace-nowrap text-right"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.ipDetail.uniqueVisitors"))}</th><th class="py-3 pr-4 whitespace-nowrap text-right"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.visits"))}</th><th class="py-3 pr-4 whitespace-nowrap text-right"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.ipDetail.registeredUsers"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.country"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.regionCity"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.device"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.browser"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.os"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.firstSeen"))}</th><th class="py-3 pr-4 whitespace-nowrap"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.lastSeen"))}</th><th class="py-3 pr-4 whitespace-nowrap text-right"${_scopeId}>${ssrInterpolate(unref(t)("admin.stats.visitorDetail.actions"))}</th></tr></thead><tbody${_scopeId}><!--[-->`);
              ssrRenderList(unref(modalRows), (item) => {
                _push2(`<tr class="border-b border-gray-100 dark:border-gray-900/80"${_scopeId}><td class="py-3 px-4 text-gray-700 dark:text-gray-200 font-mono text-xs"${_scopeId}>${ssrInterpolate(item.ip || "Local")}</td><td class="py-3 pr-4 whitespace-nowrap"${_scopeId}><div class="flex items-center gap-1.5"${_scopeId}>`);
                _push2(ssrRenderComponent(_component_UBadge, {
                  color: unref(getSourceTypeColor)(item.sourceType),
                  variant: "subtle",
                  size: "xs"
                }, {
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    if (_push3) {
                      _push3(`${ssrInterpolate(formatSourceLabel(item.sourceType || "direct"))}`);
                    } else {
                      return [
                        createTextVNode(toDisplayString(formatSourceLabel(item.sourceType || "direct")), 1)
                      ];
                    }
                  }),
                  _: 2
                }, _parent2, _scopeId));
                if (item.source && item.source !== "direct" && item.source !== item.sourceType) {
                  _push2(`<span class="text-gray-700 dark:text-gray-200 text-xs font-medium max-w-[120px] truncate"${ssrRenderAttr("title", item.source)}${_scopeId}>${ssrInterpolate(item.source)}</span>`);
                } else {
                  _push2(`<!---->`);
                }
                if (item.campaign) {
                  _push2(`<span class="text-gray-400 dark:text-gray-500 text-[11px] font-mono max-w-[80px] truncate"${ssrRenderAttr("title", item.campaign)}${_scopeId}> (${ssrInterpolate(item.campaign)}) </span>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`</div></td><td class="py-3 pr-4 text-gray-600 dark:text-gray-300 text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.visitorCount))}</td><td class="py-3 pr-4 text-gray-900 dark:text-white text-right tabular-nums"${_scopeId}>${ssrInterpolate(formatNumber(item.visitCount))}</td><td class="py-3 pr-4 text-right tabular-nums"${_scopeId}>`);
                _push2(ssrRenderComponent(_component_UBadge, {
                ...
                unref(modalSource) === "visitors" ? (openBlock(), createBlock("table", {
                      ...
                      return openBlock(), createBlock("tr", {
                        ...
                        createVNode("td", { class: "py-3 pr-4 whitespace-nowrap" }, [
                          createVNode("div", { class: "flex items-center gap-1.5" }, [
                            ...
                            item.firstSource && item.firstSource !== "direct" && item.firstSource !== item.firstSourceType ? (openBlock(), createBlock("span", {
                              key: 0,
                              class: "text-gray-700 dark:text-gray-200 text-xs font-medium max-w-[120px] truncate",
                              title: item.firstSource
                            }, toDisplayString(item.firstSource), 9, ["title"])) : createCommentVNode("", true),
                            item.firstCampaign ? (openBlock(), createBlock("span", {
                              key: 1,
                              class: "text-gray-400 dark:text-gray-500 text-[11px] font-mono max-w-[80px] truncate",
                              title: item.firstCampaign
                            }, " (" + toDisplayString(item.firstCampaign) + ") ", 9, ["title"])) : createCommentVNode("", true)
                          ])
                        ]),
                  ...
                  createVNode("thead", { class: "sticky top-0 bg-white dark:bg-[#121214]" }, [
                    createVNode("tr", { class: "text-left text-gray-500 border-b border-gray-200 dark:border-gray-800" }, [
                      ...
                      createVNode("th", { class: "py-3 pr-4 whitespace-nowrap" }, toDisplayString(unref(t)("admin.stats.source")), 1),
                      ...
                      return openBlock(), createBlock("tr", {
                        ...
                        createVNode("td", { class: "py-3 pr-4 whitespace-nowrap" }, [
                          createVNode("div", { class: "flex items-center gap-1.5" }, [
                            ...
                            item.source && item.source !== "direct" && item.source !== item.sourceType ? (openBlock(), createBlock("span", {
                              key: 0,
                              class: "text-gray-700 dark:text-gray-200 text-xs font-medium max-w-[120px] truncate",
                              title: item.source
                            }, toDisplayString(item.source), 9, ["title"])) : createCommentVNode("", true),
                            item.campaign ? (openBlock(), createBlock("span", {
                              key: 1,
                              class: "text-gray-400 dark:text-gray-500 text-[11px] font-mono max-w-[80px] truncate",
                              title: item.campaign
                            }, " (" + toDisplayString(item.campaign) + ") ", 9, ["title"])) : createCommentVNode("", true)
                          ])
                        ]),
