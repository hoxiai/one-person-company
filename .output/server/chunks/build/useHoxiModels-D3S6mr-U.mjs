...
import { t as useSettings, y as useFetch } from './server.mjs';
...
function injectModelReferences(html, models, options) {
  ...
  return html.replace(
    ...
    (_match, slug) => {
      ...
      return `<a href="${escapeHtml(options.href(model.slug))}" class="not-prose inline-flex flex-wrap items-baseline gap-x-2 no-underline"><span class="font-medium text-slate-700">${escapeHtml(model.name)}</span><span class="text-xs tabular-nums text-slate-500">${escapeHtml(options.blendedLabel)} \xA5${formatPrice(model.blendedPrice)}${score}</span></a>`;
    }
  );
}
